...
\begin{myframe}[Motivation]
  \note[item]{Motivieren mit einem use case}
...
\begin{myframe}[Fundamentals - Pose Estimation]
    \note[item]{Überlappung und Verdeckung Probleme, nicht nur bei mehreren Personen pro Bild}
    \begin{figure}
...
\begin{myframe}[Pose Estimation - Shallow Methods]
    \note[item]{Ansatz: Menschliches Skelet besteht aus Strukturen (Gliedmaßen) welche mit "Federn" verbunden sind.}
...
\begin{myframe}[Pose Estimation - Deep Methods]
    ...
    \note[item]{Geben keine Begründung für die Residual Modules. Normalerweise z.B. damit kein vanishing gradient auftritt}
    \note[item]{Keine Aktivierungsfunktionen angegeben, also keine Ahnung warum kein ReLU}
    \begin{columns}[T]
        \begin{column}{.48\textwidth}
            \begin{itemize}
...
\begin{myframe}[Stacking architectures - Qualitative evaluations]
    ...
    \note[item]{Selbst wenn noisy trotzdem von Vorteil.}
    \begin{figure}
        \includegraphics[width=0.75\textwidth]{context-belief-maps.png}
        \caption{Easier to detect features like head and neck can influence the detection of harder features like elbows and knees. Thus, an inititally wrong result (see \textit{R. Elbow} in Stage 1) can be corrected in later stages.\fcite{newell_stacked_2016}}
    \end{figure}
    \begin{figure}
...
\begin{myframe}[Method - Joint methods]
    ...
    \note[item]{Argmax als postprocessing schritt nötig damit exakte Koordinaten. Wenn man end-to-end trainieren will ist ein nicht-differentierbares Argmax aber problematisch.}
...
\begin{myframe}[Multitask Deep HAR - Architecture]
    ...
    \note[item]{Nach letztem prediction block: Heatmap (Softargmax) und Wahrscheinlichkeitsvektor}
