...
The authors also evaluate the effect of adding their representation to other previous approaches and find that adding it to the previous state-of-the-art approach by \cite{carreira_quo_2017} (see \sref{sec:two-stream}) they were able to improve the accuracy slightly by $0.5$ percentage points using estimated poses on the JHMDB dataset.
When using ground truth annotation for the joint positions, the authors observe an improvement of $3$ percentage points upon \cite{carreira_quo_2017}, indicating that their representation is complementary to the Two-Stream approach and that it is worthwhile to improve upon the pose estimation component.
