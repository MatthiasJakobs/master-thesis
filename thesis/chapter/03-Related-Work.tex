...
\subsubsection{Cascade Feature Aggregation for Human Pose Estimation}
\cite{su_cascade_2019}

% Introduction
%   - Based on Hourglass
%   - Key Difference: Input and Output from previous stage taken as input for current stage

Recently, \cite{su_cascade_2019} proposed a model, which is based on the Stacked Hourglass architecture \cite{newell_stacked_2016} \sref{sec:hourglass}.
The key difference is that the output of the previous hourglass is combined with multiple intermediate results from the Hourglass to form the input of the next hourglass.
In addition, the intermediate heatmaps are aggregated to further improve the overall performance of the network.

% Architecture
Create heatmap by using a gaussian where the mean is placed on the ground truth coordinate.

Computing final heatmap by averaging intermediate heatmaps.

% Evaluation

For evaluation, the authors meassure PCKh scores on the MPII and LSP dataset.
