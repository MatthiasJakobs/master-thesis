...
The authors additionally evaluated their model on the Buffy and Iterative Image Parse datasets, presented in Section \ref{sec:andriluka-eval}.
...
- Towards Understanding Action Recognition
	- Up to here: Action recognition on simple, often lab datasets, works fine but struggle in real life situations.
	- The authors took the HMDB dataset, which is a challenging dataset and annotated the pose of a subset of the dataset. With annotated poses, ground truth optical flow and bounding boxes can be computed (need to define what optical flow is before). By substituting ground truth information into the pipeline of a state-of-the-art algorithm (Dense Trajectories, if already talked about refer back) the authors studied how much these individual features contribute to classification accuracy.
	- The authors define a hierachie of features. First, low level features include optical flow as well as pixel gradients. These features can used to compute mid level featues such as image descriptors. Finally, pose is defined as a high level feature since they can be interpreted semantically (need citation since its a quote). 
	- Find that Pose information is one of the most important feature for Action Recognition
	- Baseline: Dense trajectories features with a RBF kernel SVM.
	- TODO: Describe what puppet flow and puppet mask is
	...
	- Impact of flow:
		- The highest accuracy gains (11 pp over baseline) were achieved by using the ground truth flow masked with the human segmentation (puppet mask) in combination with using the estimated flow around the puppet mask (put graphic from paper here). The authors argue that is most likely due to the fact that the separation between the background flow and subject flow is more distinct, resulting in a sharp motion boundary. Significant improvements on all descriptors besides HOG. Highest improvement in MBH (16pp) (which is logical)
	- Impact of masking subject:
		- Next, the authors investigate the impact of masking the subject. They use two types of masking. First, they use a bounding box around the subject. Second, they utilize the puppet mask from earlier. When applying the masking in such a way that the image is masked before computing the features (as opposed to densely computing features and then masking afterwards) they find that both bounding box masking and puppet flow masking improve significantly upon the baseline (5.6 and 8 pp respectively). They argue that puppet mask likely leads to a better accuracy because the optical flow algorithm is easier to compute on the subjects boundary. In addition, they utilize a person detector from (TODO: cite here) to compute the bounding box but find that the computed bounding box is not accurate enough to improve significantly upon the baseline.
	- Impact of pose
		- The authors generate two types of pose related feature. First, they use the joint positions over time as trajectories. Second, they compute additional features by calculating the distances and relative angles between all joint pairs. In addition, they iterate every triplet of joints and calucalte the angle spanned by the two relative vectors between joints anchored at the third joint. Besides this, they encode temporal information by computing the change in these values for consecutive frames. (TODO: put graphic from paper here).
		- They find that the accuracy improves by 19pp over the baseline, suggesting that pose information is by far the best feature to use for action recognition (cite because quote). Also, adding the image features to the pose features does not significantly improve upon the accuracy. The authors thus argue that, once pose features are used, the image feature contribution is negligable.
		- The authors additionaly evaluate the use of estimated poses using the approach presented by (cite yang and ramanan). They find that, while the estimated pose is of low quality, this approach still outperforms Dense trajectories baseline approach, suggesting that even low quality pose information is relevant for action recognition. 

- P-CNN
	- The authors use Convolutional Neural Networks for feature extraction. Specifically, they use a Two-Stream approach (cite zissermann et al) where the network is split into two parallel parts. The first part processes the individual RGB frames, resulting in an appearance feature vector. The second part processes flow images and outputs another feature vector. These two vectors are then concatenated, which results in the final feature vector for the entire video clip. Each part consists of 5 convolutional layers, followed by 3 fully-connected layers. The RGB part is pretrained using ImageNet ILSVRC-2012 (cite) while the flow part is pretrained using the UCF101 dataset (cite). Once pretrained, the last fully-connected layer of both parts is removed, resulting in a output size of the previous layer of 4096, which they use as the extracted feature. (TODO: show image of network)
	- First, the authors compute optical flow (cite High accuracy optical flow estimation based on a theory for warping) for consecutive frames in each video clip beforehand. They scale the x and y flow values to the interval [0, 255]. Then, they concatenate these two flow maps and add a third map with the flow magnitude values, resulting in a 3 channel flow image.
	- They also preestimate the joint positions using the approach by (cite Yang ramanan). Additionally, they use a dynamic programming approach by (cite [8]) to refine the estimated pose by incorporating flow information as well as temporal information from previous and following poses.
	...
	- After passing all subimages through the network, the resulting feature vectors $f_t^p$ for time step $t$ and body part $p$ are aggregated by first computing minimum and maximum values $m_i, M_i$ for each descriptor dimension $i$. Then, the minimum values for each dimension are concatenated, followed by another concatenation of the maximum values. The authors refer to this vector as the static video descriptor since it is computed per frame without taking future or past frames into account. They compute another descriptor, called the dynamic video descriptor, where the original feature descpritor $f_t^p$ gets subtracted by $f_(t-4)^p$, thus encoding temporal information. The aggregation of minimum and maximum values following that is identical to the aggregation for the static video descriptor. Finally, both descriptors are again concatenated and normalized. See figure XX for a visualization.
	- The resulting feature vector is then used as input for a linear Support Vector Machine, used for action classification.
	- When training on the JHMDB dataset, the authors find that their approach does not outperform HLPF (CITE) when using ground truth pose annotations (74,6 vs 77,8 percent accuracy). However, when using estimated pose, their approach significantly outperforms HLPF (61,1 vs 25,3). While the authors give no explanation for why this might be the case, one possibility would be that the conclusion of (cite) that visual features and flow do not improve upon accuracy when also utilizing pose might be wrong. Also, since the authors use a different approach for estimating pose and do not evaluate the accuracy of the pose estimator individually it might be the case that the pose information itself is not sufficient for the pose features computed by HLPF to be accurate enough.
	- Interestingly, the authors find that, while their approach does not outperform the state of the art presented in (cite improved DT), a combination of their descriptor with IDT significantly outperfomrs IDT on its own. The authors argue that, because IDT uses a larger time window for feature aggregation, actions like "kick_ball" achieve higher accuracy while on actions like "shoot_gun" and "wave" the image features extracted by their method provide more information and thus higher accuracy. A combination thus improves the accuracy in both scenarios simultaniously.
	- TODO: Explain Fisher Vector (Improving the
