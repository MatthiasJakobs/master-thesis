...
The authors use \textit{Histogram of Oriented Gradients (HoG)} \cite{dalal_histograms_2005}, \textit{Histogram of Optical Flow (HoF)} \cite{laptev_learning_2008} (see Sec. \ref{sec:laptev-shallow} for more information) as well as \textit{Motion Boundary Histograms (MBH)} \cite{dalal_human_2006} to extract features along the trajectoties.
...
In addition, they compared their findings against \cite{laptev_learning_2008}, discussed in Sec. \ref{sec:laptev-shallow}.
...
In \cite{jhuang_towards_2013}, the authors evaluate the effect of different parts of a state-of-the-art action recognition pipeline \cite{wang_dense_2013} in order to gain a deeper understanding of the problem (see Section \ref{sec:dense-trajectories} for a detailed explanation of the algorithm).
...
The authors compare their results to the descriptor presented in Section \ref{sec:jhuang-towards} (from now on referred to as \textit{HLPF} for High Level Pose Features) as well as to the Improved Dense Trajectories (\textit{IDT}) descriptor presented in Section XX, which was the state-of-the-art approach at that time. %TODO: Backreferenz
