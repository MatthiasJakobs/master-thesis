...
The augmentation is identical to the augmentation mentioned in \sref{sec:exp-mpii}. % TODO: Augmentation changed
...
Also, the approach for computing ground truth bounding boxes, as well as preprocessing the \textit{fragments} and augmentations is identical to the one presented earlier. % TODO: augmentation changed
...
% TODO: Evaluate using learning rate scheduling or remove the "initial"
As discussed before, for evaluating a model on the test dataset, the estimations need to be send to the Max Planck institute for evaluation.
...
\begin{table}[]
    ...
    \end{tabular}
    \caption{Different model configurations and their corresponding PCKh validation score. The $p$-values were computed using a randomization test. $p$-values below the significance level of $0.05$ are shown in bold.} % TODO
    \label{tab:mpii_results}
\end{table}

Next, the best performing model ($8$ prediction blocks, $2$ context heatmaps) was used to estimate the pose on the test dataset and the results were submitted to the Max Planck institute.
...
% TODO: Test on Penn action using PCK @ 0.2 and @ 0.1
