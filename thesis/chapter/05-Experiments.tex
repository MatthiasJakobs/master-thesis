...
To compute the image coordinates, a multiplication with the width and height of the input image as well as a rounding step is necessary, possibly introducing rounding errors.

In the first experiment, synthetic images of size $255 \times 255$ pixels are created, since this is the size of the input images to the network after preprocessing.
At each $x,y$ position, a two-dimensional gaussian with mean $(x,y)$ and covariance $c$ is placed.
Second, the expectations are computed using the Soft-argmax function.
Third, the expectations are compared to the ground truth mean value.
We found that the Soft-argmax function accurately regresses the true expectation for small covariances $c$.
...
The images in \fref{fig:softargmax_variance_test} are created using $d=2$ to allow for rounding errors.
As can be seen, the Soft-argmax function gets less accurate the higher the covariance is.
Also, accuracy decreases the closer the mean of the gaussian is to the border of the image.
...
In the second experiment, we analyze the accuracy quantitatively to evaluate the impact of the previously observed behaviour on actual data.
Synthetic joint heatmaps are generated by placing gaussians at the position of the ground truth pose coordinates of a subset of the MPII dataset.
The experiment is conducted on $1000$ random images from the MPII dataset.
The distance between the computed coordinate and ground truth coordinate is computed with different covariance values $c$.
...
\begin{table}[]
    ...
    \hline
        ...
        \begin{tabular}{@{}c@{}} 8 \\ 8 \end{tabular} & \begin{tabular}{@{}c@{}} 0 \\ 2 \end{tabular} & \begin{tabular}{@{}c@{}} 86.71 \\ 87.00  \end{tabular} & 0.73  \\ \hline
        ...
        \begin{tabular}{@{}c@{}} 4 \\ 8 \end{tabular} & \begin{tabular}{@{}c@{}} 2 \\ 2 \end{tabular} & \begin{tabular}{@{}c@{}} 85.64 \\ 87.00  \end{tabular} & 0.12  \\ \hline
        ...
        \begin{tabular}{@{}c@{}} 2 \\ 8 \end{tabular} & \begin{tabular}{@{}c@{}} 2 \\ 2 \end{tabular} & \begin{tabular}{@{}c@{}} 84.01 \\ 87.00  \end{tabular} & \textbf{0.001}  \\ \hline            
    \end{tabular}
...
Afterwards, they mirrored the image horizontally, computed the pose on the mirrored version and mirrored the resulting pose once more.
...
The final pose is then computed by taking the average of each joint position per image over all computed poses.
The authors do not describe this method of displacement in detail, which might cause the difference in test accuracy reported.
...
\begin{table}[]
    ...
    \hline
        ...
        0.84 & 0.86 & 0.85 & 0.85 & 0.87 & 0.86 & 0.88 & 0.87 \\ \hline \hline
    \end{tabular}}
