...
Each layer $N_l$ consists of $m_l \in \mathbb{N}$ perceptrons.
Consecutive layers are connected by directed, weighted edges $e_{ij} \in \mathbb{R}$ with $n_i \in N_l$ and $n_j \in N_{l+1}$ so that each perceptron $n_i$ is connected to the input of each perceptron $n_j$ in the consecutive layer.
...
The first layer behaves just like in the previous example with each perceptron splitting the input space into two.
